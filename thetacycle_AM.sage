...
def thetacycle(p, k, ord, includek = True, errorchecking = False):
	...
	# 
	# PLEASE NOTE: Edixhoven assumes that the weight filtration of the form 
	# you start with is exactly k!
	# So if k = p or k = p + 1, but the form you have in mind actually has 
	# weight filtration 1 or 2, the resulting theta cycle will not be correct!
	#
	if (p == 2 and k > 4) or (p > 2 and k > p + 1) or (k < 1):
